...
Selected-model context comes from gateway catalog metadata or an explicit per-model override. Claude Open accepts context-capacity fields such as `context_length`, `context_window`, `max_context_tokens`, and nested context/limit objects. It also checks the optional same-gateway `/api/models` details catalog by default and merges it by provider/real model id; set `modelDetailsEndpoint` to another relative path or `null` when a gateway uses a different contract. This matters when `/v1/models` publishes a conservative input-compatible value while the details catalog exposes the real context window. `max_input_tokens` remains a separate input limit and is never mislabeled as model context. Claude Open does not invent a context size when neither catalog nor an override provides one; that case is labeled **Not reported**.
