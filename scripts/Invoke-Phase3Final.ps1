...
<#
.SYNOPSIS
  Phase 3 LIVE DECISIVE end-to-end verification (p03final). Proves the user's real
  complaints are fixed: no stale 8788, live adapter port in the FLAT 3P config,
  purged stale configLibrary, family-tier probes resolving to a HEALTHY opus, and
  the GENUINE full-trust WindowsApps claude.exe reaching 127.0.0.1 with NO
  ERR_CONNECTION_REFUSED.

.DESCRIPTION
  Adapted from Invoke-Phase3Tier.ps1. Two-pass adapter (pass-1 reads /v1/models to
  pick the healthy opus; pass-2 restarts with CLAUDE_OPEN_HEALTHY_DEFAULT set). The
  isolated 3P config is written against the SECOND adapter's ACTUAL bound ephemeral
  port + token via the production write-3p-config path with --assign-family-tiers.

  DECISIVE verifications added over the tier variant:
    (a) configLibrary/<uuid>.json inferenceGatewayBaseUrl == http://127.0.0.1:<ACTUAL_PORT>
        and NO literal 8788 anywhere in the written config.
    (b) inferenceModels has 38+ entries with the healthy opus FIRST.
    (c) every entry carrying anthropicFamilyTier (haiku/sonnet/opus) points at a
        HEALTHY opus alias with isFamilyDefault:true.
    (d) deploymentMode '3p' in claude_desktop_config.json.
  ...
  profile. All evidence under test-results\corrective\<run>\p03final\ (git-ignored).
#>
[CmdletBinding()]
param(
  [string]$EvidenceRoot,
...
$p03Path = Join-Path $runPath 'p03final'
...
$result = [ordered]@{
  ...
  evidenceDir = $p03Path
  launchPath = 'production (createProductionWorkspace) + --assign-family-tiers + CLAUDE_OPEN_HEALTHY_DEFAULT + genuine full-trust WindowsApps claude.exe'
  adapter = [ordered]@{
  ...
  # DECISIVE config-file verifications (task step 3).
  configLibraryVerify = [ordered]@{
    activeConfigFile = $null
    baseUrlInConfig = $null
    baseUrlIsLivePort = $false
    baseUrlIsStale8788 = $null
    noLiteral8788Anywhere = $false
    inferenceModelCount = 0
    modelCountAtLeast38 = $false
    firstModelAlias = $null
    firstModelIsHealthyOpus = $false
  ...
  familyTierTags = [ordered]@{
    ...
    tiersObserved = @()
    tierToAlias = [ordered]@{}
    allTiersPointToHealthyOpus = $false
  ...
  expectedVsActual = @()
  humanRenderNote = 'A fully user-VISIBLE rendered chat turn still requires a human typing in the UI. The strongest AUTOMATED signal captured here is: client REACHED adapter (no ERR_CONNECTION_REFUSED) + ConfigHealth reachable + the client OWN activation inference returned HTTP 200.'
}
...
try {
  ...
  if (-not (Wait-ForFile $runtimeFile1 45)) {
    $result.status = 'NOT RUN'
    $result.rootCause = 'Pass-1 adapter did not write runtime.json within 45s (config invalid, secret missing, or first-run). See adapter-pass1.stderr.log.'
...
'@
  ...
  # === Step 2/3: write the 3P config via PRODUCTION path + FAMILY TIERS ===
  ...
  # === Step 3 DECISIVE: inspect the written config-library file ===
  ...
  if ($activeConfigFile) {
    ...
    # (a) live base URL, NOT 8788, no literal 8788 anywhere in the file.
    $result.configLibraryVerify.baseUrlInConfig = [string]$rawConfig.inferenceGatewayBaseUrl
    $result.configLibraryVerify.baseUrlIsLivePort = [bool]($rawConfig.inferenceGatewayBaseUrl -eq $baseUrl)
    $result.configLibraryVerify.baseUrlIsStale8788 = [bool]($rawConfig.inferenceGatewayBaseUrl -match ':8788(\b|/)')
    $result.configLibraryVerify.noLiteral8788Anywhere = [bool](-not ($configText -match '8788'))

    # (b) 38+ inferenceModels, healthy opus FIRST.
    $result.configLibraryVerify.inferenceModelCount = $allModels.Count
    $result.configLibraryVerify.modelCountAtLeast38 = [bool]($allModels.Count -ge 38)
    $firstAlias = $null
    if ($allModels.Count -gt 0) { $firstAlias = [string]$allModels[0].name }
    $result.configLibraryVerify.firstModelAlias = $firstAlias
    $result.configLibraryVerify.firstModelIsHealthyOpus = [bool]($firstAlias -and ($firstAlias -eq $defaultAlias) -and ($firstAlias -match '(?i)opus') -and ($knownOverloaded -notcontains $firstAlias))

    # (c) family-tier tags: every anthropicFamilyTier entry -> HEALTHY opus + isFamilyDefault.
    $tiersObserved = @()
    $taggedCount = 0
    $haikuAlias = $null
    $isFamilyDefaultPresent = $false
    $tierToAlias = [ordered]@{}
    $allTiersHealthyOpus = $true
    $tierTaggedSeen = $false
    foreach ($im in $allModels) {
      $hasTier = ($im.PSObject.Properties.Name -contains 'anthropicFamilyTier') -and $im.anthropicFamilyTier
      if ($hasTier) {
        $tierTaggedSeen = $true
        $taggedCount += 1
        $tierName = [string]$im.anthropicFamilyTier
        $aliasName = [string]$im.name
        if ($tiersObserved -notcontains $tierName) { $tiersObserved += $tierName }
        if (-not $tierToAlias.Contains($tierName)) { $tierToAlias[$tierName] = $aliasName }
        if ($tierName -eq 'haiku') { $haikuAlias = $aliasName }
        $thisHealthyOpus = ($aliasName -match '(?i)opus') -and ($knownOverloaded -notcontains $aliasName)
        $thisIsFamilyDefault = (($im.PSObject.Properties.Name -contains 'isFamilyDefault') -and $im.isFamilyDefault -eq $true)
        if (-not ($thisHealthyOpus -and $thisIsFamilyDefault)) { $allTiersHealthyOpus = $false }
      }
      if (($im.PSObject.Properties.Name -contains 'isFamilyDefault') -and $im.isFamilyDefault -eq $true) { $isFamilyDefaultPresent = $true }
    }
    if (-not $tierTaggedSeen) { $allTiersHealthyOpus = $false }
    ...
    $result.familyTierTags.tierToAlias = $tierToAlias
    $result.familyTierTags.taggedItemCount = $taggedCount
    $result.familyTierTags.isFamilyDefaultPresent = $isFamilyDefaultPresent
    $result.familyTierTags.allTiersPointToHealthyOpus = [bool]$allTiersHealthyOpus
    ...
    $result.configLibraryVerify.deploymentMode3pInDesktopConfig = $deploymentMode3pWritten

    # Redacted config dump to evidence.
    $redactedModels = @()
    foreach ($im in $allModels) {
  ...
  # === Step 4: launch the GENUINE full-trust WindowsApps claude.exe ===
  if (-not (Test-Path -LiteralPath $ClientExe -PathType Leaf)) {
    ...
    $result.client.exeKind = 'genuine full-trust WindowsApps signed claude.exe (1.20186.1.0); NOT AppContainer, reaches 127.0.0.1 with no loopback exemption'
  ...
  # === Step 5a: parse request counters + message outcomes + tier-reconcile ===
...
'@
  ...
  if ($counterObj) {
    ...
    $result.clientOriginated.reachedAdapter = [bool]([int]$counterObj.clientEventCount -gt 0)
  ...
  # === Step 5b: read the ISOLATED profile main.log ONLY ===
  $clientLogSources = @()
  ...
  foreach ($log in $clientLogSources) {
    foreach ($line in @(Get-Content -LiteralPath $log.FullName -ErrorAction SilentlyContinue)) {
      $rawClientLines += $line
      if ($line -match '(?i)deploymentMode|onboarding|msix_required|cowork|3p|1p|first-?party|firstParty|custom.?3p|inference|model|picker|ConfigHealth|apiHost|reachable|unreachable|refused|ERR_CONNECTION|127\.0\.0\.1|localhost') {
        $safeClientLines += (Protect-Line $line)
      }
    }
  }
  [System.IO.File]::WriteAllLines((Join-Path $p03Path 'client-main-sanitized.log'), $safeClientLines, $utf8NoBom)
  ...
  # Does apiHost in the log match the ACTUAL bound port (no stale 8788)?
  $result.client.connectionRefusedObserved = [bool]($joinedClient -match '(?i)ERR_CONNECTION_REFUSED|connection refused|ECONNREFUSED')
  if ($joinedClient -match ('(?i)(apiHost|base ?url|inferenceGatewayBaseUrl)["'':\s]{0,6}[^\r\n]{0,40}:' + [string]$port + '\b')) {
    $result.client.apiHostMatchesActualPort = $true
  } elseif ($joinedClient -match '(?i)(apiHost|base ?url).{0,40}:8788\b') {
    $result.client.apiHostMatchesActualPort = $false
  } elseif ($joinedClient -match ('(?::' + [string]$port + '\b)')) {
    $result.client.apiHostMatchesActualPort = $true
  }
  ...
  $clientReachedAdapter = [bool]$result.clientOriginated.reachedAdapter
  ...
  elseif ($clientReachedAdapter -and -not $result.client.connectionRefusedObserved) { $configHealthReachable = $true }
  ...
  $checks = @(
    ...
    [ordered]@{ requirement = 'Config base URL == live port (NOT 8788)'; expected = $baseUrl; actual = ($result.configLibraryVerify.baseUrlIsLivePort -and -not $result.configLibraryVerify.baseUrlIsStale8788) },
    [ordered]@{ requirement = 'No literal 8788 anywhere in written config'; expected = 'absent'; actual = $result.configLibraryVerify.noLiteral8788Anywhere },
    [ordered]@{ requirement = 'inferenceModels >= 38 with healthy opus FIRST'; expected = '>=38 & opus first'; actual = ($result.configLibraryVerify.modelCountAtLeast38 -and $result.configLibraryVerify.firstModelIsHealthyOpus) },
    [ordered]@{ requirement = 'All family tiers -> healthy opus + isFamilyDefault'; expected = 'haiku/sonnet/opus all healthy opus'; actual = $result.familyTierTags.allTiersPointToHealthyOpus },
    [ordered]@{ requirement = 'deploymentMode 3p in claude_desktop_config.json'; expected = '3p'; actual = $result.configLibraryVerify.deploymentMode3pInDesktopConfig },
    [ordered]@{ requirement = 'Client entered custom 3P mode'; expected = 'custom-3p activation line'; actual = $result.client.custom3pActive },
    [ordered]@{ requirement = 'Client REACHED adapter (no ERR_CONNECTION_REFUSED)'; expected = 'any client-originated request'; actual = $clientReachedAdapter },
    [ordered]@{ requirement = 'ConfigHealth reachable'; expected = 'reachable'; actual = $configHealthReachable },
    [ordered]@{ requirement = "Client's activation inference returned 200"; expected = '2xx at adapter (client window)'; actual = $result.clientOriginated.probeModelReturned200 }
  )
  $result.expectedVsActual = $checks

  # gatewayConnectionFixed = the decisive user complaint: no more ERR_CONNECTION_REFUSED.
  $result.gatewayConnectionFixed = [bool]($clientReachedAdapter -and -not $result.client.connectionRefusedObserved)
  # modelsWork = default healthy opus + tiers resolve + activation 200 (or reconcile to healthy).
  $result.modelsWork = [bool]($result.defaultSelection.chosenIsHealthyOpus -and $result.familyTierTags.allTiersPointToHealthyOpus -and $result.clientOriginated.probeModelReturned200)

  if ($clientDrove3p -and $clientReachedAdapter -and $configHealthReachable -and $result.clientOriginated.probeModelReturned200) {
    $result.status = 'PASS'
    ...
    $result.verdict = 'PASS: genuine full-trust WindowsApps client entered 3P, REACHED the adapter on the LIVE port (no ERR_CONNECTION_REFUSED / no stale 8788), ConfigHealth reachable, and its OWN activation inference returned HTTP 200 (not 503). Resolution: ' + $resolutionPath + '.'
    $result.remainingGap = 'None (automated). A human-visible rendered chat turn still requires a person typing in the UI.'
    $result.rootCause = $null
  } elseif ($clientDrove3p -and $clientReachedAdapter -and $result.clientOriginated.postMessages -and -not $result.clientOriginated.probeModelReturned200) {
    $result.status = 'PARTIAL'
    $statusList = (@($result.clientOriginated.messageStatuses) | ForEach-Object { $_.status }) -join ','
    $result.verdict = 'PARTIAL: client REACHED the adapter (connection fixed) and drove POST /v1/messages, but the adapter returned non-2xx (statuses: ' + $statusList + '). Connection problem is fixed; activation inference did not return 200.'
    $result.remainingGap = 'Client-originated activation inference returned non-2xx: ' + $statusList + '. Connection/port fix is proven; inference resolution still failing.'
    $result.rootCause = 'Client-originated activation inference returned non-2xx: ' + $statusList
  } elseif ($clientDrove3p -and $clientReachedAdapter -and -not $result.clientOriginated.postMessages) {
    $result.status = 'PARTIAL'
    $result.verdict = 'PARTIAL: client entered 3P and REACHED the adapter (connection fixed, no ERR_CONNECTION_REFUSED) but did not emit a POST /v1/messages in the wait window (build may defer first inference until a human sends a message). No 503.'
    $result.remainingGap = 'No client-originated POST /v1/messages observed within the wait window; a human message may be needed to trigger first inference. Connection/config/port fix is proven.'
    $result.rootCause = 'No client-originated POST /v1/messages observed at the adapter within the client window.'
  } else {
    $result.status = 'FAIL'
    $reasons = @()
    if (-not $clientDrove3p) { $reasons += 'client did not enter genuine 3P mode (custom-3p=' + $result.client.custom3pActive + ', deploymentMode=' + $result.client.deploymentMode + ', onboarding=' + $result.client.firstPartyOnboardingObserved + ')' }
    if (-not $clientReachedAdapter) { $reasons += 'CLIENT did NOT reach the adapter (no client-originated request; possible ERR_CONNECTION_REFUSED=' + $result.client.connectionRefusedObserved + ')' }
    ...
    $result.remainingGap = $result.rootCause
    $result.verdict = 'FAIL: ' + $result.rootCause
  }
}
catch {
  $result.status = 'NOT RUN'
  ...
  Write-Host ('Phase 3 FINAL evidence: ' + $p03Path)
  Write-Host ('Status: ' + $result.status)
  Write-Host ('ACTUAL bound port: ' + $result.adapter.actualBoundPort)
  Write-Host ('Config base URL live-port (not 8788): ' + $result.configLibraryVerify.baseUrlIsLivePort + ' | no-literal-8788: ' + $result.configLibraryVerify.noLiteral8788Anywhere)
  Write-Host ('inferenceModels: ' + $result.configLibraryVerify.inferenceModelCount + ' | opus-first: ' + $result.configLibraryVerify.firstModelIsHealthyOpus)
  Write-Host ('Family tiers all->healthy opus: ' + $result.familyTierTags.allTiersPointToHealthyOpus + ' | tiers=' + ($result.familyTierTags.tiersObserved -join ','))
  Write-Host ('Client 3P: custom3p=' + $result.client.custom3pActive + ' deploymentMode=' + $result.client.deploymentMode)
  Write-Host ('Client REACHED adapter: ' + $result.clientOriginated.reachedAdapter + ' | ERR_CONNECTION_REFUSED=' + $result.client.connectionRefusedObserved)
  Write-Host ('ConfigHealth reachable: ' + $result.client.configHealthReachable + ' state=' + $result.client.configHealthState)
  ...
  Write-Host ('gatewayConnectionFixed: ' + $result.gatewayConnectionFixed + ' | modelsWork: ' + $result.modelsWork)
  if ($result.verdict) { Write-Host ('Verdict: ' + $result.verdict) }
  if ($result.remainingGap) { Write-Host ('Remaining gap: ' + $result.remainingGap) }
}
